 ...
 */
void SparseMatrix_CCS::ConvertFromMatrix(const helpers::matrix &dense)
{
   this->n = dense.getn();
   this->m = dense.getm();
   col.resize(m+1);
 ...
 */
void SparseMatrix_CCS::ConvertToMatrix(helpers::matrix &dense) const
{
   dense = 0;
 ...
 */
void SparseMatrix_CCS::prod(const helpers::matrix &A, const SparseMatrix_CCS &B)
{
   assert(A.getm() == B.gn());
   this->n = A.getn();
   this->m = B.gm();
